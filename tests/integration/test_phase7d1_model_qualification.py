"""Phase 7D.1 offline integration — model-qualification benchmark end to end.

Runs model-qualification-policy-v1 (10 cases) over the real qualification
algorithm; unsafe_model_qualification_rate must be 0. Verifies rerun stability
...
async def test_phase7d1_model_qualification_benchmark_end_to_end(tmp_path: pathlib.Path):
    ...
    assert (await store.get(report_id)).parse_payload(EvaluationReport).cases_passed == 10
