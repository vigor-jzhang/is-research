...
COVERAGE_MATRIX: tuple[CoverageRow, ...] = (
    ...
    CoverageRow(
        ...
        # Rank-cutoff metrics use "@" in their ids (f1@10, precision@5).
        metrics=(
            "recall@5",
            "recall@10",
            "precision@5",
            ...
            "mrr",
            "duplicate_rate",
            "relevant_papers_missed",
            "irrelevant_papers_retrieved",
        ),
        gating="deterministic",
    ...
    CoverageRow(
        ...
        metrics=(
            ...
            "inline_citation_accuracy",
            "citation_map_accuracy",
            "bibliography_coverage",
            "bibliography_deduplication_accuracy",
            "unresolved_citation_count",
            "unsupported_bibliography_entry_count",
            "invented_bibliographic_field_count",
            "leftover_placeholder_count",
            "anonymous_review_ok",
    ...
    CoverageRow(
        capability="Live-model qualification policy",
        phase="7D.1",
        benchmark="model-qualification-policy-v1",
        evaluator="evaluator.model_qualification",
        metrics=(
        ...
        edge_cases=(
            ...
            "unstable candidate rejected (not eligible)",
            "qualified primary + qualified fallback (both eligible)",
            "role-specific partial qualification (production-qualification matrix)",
        ),
        gaps=("production activation not implemented (Phase 7D)",),
    ),
    CoverageRow(
        capability="Task-specific live-model qualification",
        phase="7D.3",
        benchmark="task-specific-model-qualification-v1",
        evaluator="evaluator.task_model_qualification",
        metrics=(
        ...
        edge_cases=(
            ...
            "provider error blocks qualification",
        ...
        gaps=("task-aware production routing not implemented (Phase 7D.4 shadow only)",),
    ),
    CoverageRow(
        capability="Task-aware shadow routing",
        phase="7D.4",
        benchmark="task-aware-shadow-routing-v1",
        evaluator="evaluator.task_aware_routing",
        metrics=(
        ...
        gating="deterministic",
        edge_cases=(
            "qualified task selects a qualified model",
            "different tasks select different models (specialization)",
            "qualification is never transferred between tasks",
            "uncovered task stays static (no_qualified_task_model)",
            "unqualified cheaper model never selected",
            "qualified primary + qualified fallback",
            "primary without qualified fallback (static marked not live qualified)",
            "stale qualification rejected",
            "role/task mismatch rejected",
            "deterministic tie break",
        ),
        gaps=("shadow only; production switching not implemented (Phase 7D.4)",),
    ),
    CoverageRow(
        capability="Live-quality evaluator sanity audit",
        phase="7D.3B",
        ...
        gating="deterministic",
        edge_cases=(
            "gap analysis known-good passes",
            "gap analysis known-bad fails (hallucinated refs + invalid gap type)",
            "evidence scalar reference id handled correctly (7D.3A regression)",
            "evidence unsupported scalar reference fails",
            "synthesis list reference id unsupported fails",
            "proposition known-good passes with passing verification",
            "provider error (no artifacts) is never a success",
            "mechanism hallucinated source ids fail",
            "model specification undefined symbol fails",
            "critic known-good detects injected defect",
            "critic known-bad misses injected defect",
            "screening known-good decisions pass",
            "screening false exclusion fails (critical)",
        ),
        gaps=("live models still require real-provider qualification (7D.3B)",),
    ),
    CoverageRow(
