...
Tests use fake providers/tools and `respx` for OpenRouter; no live API calls in CI. `tests/conftest.py` ensures `pytest` without `-m live` skips live tests. 63 unit, 34 integration, and 16 opt-in live test files cover every phase:

```bash
uv run pytest                # offline: 669 passed, 19 skipped
