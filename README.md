...
uv run research-agent routing qualification remaining                    # gap_analysis now covered (gemini-3.7-flash primary)

# Gap-workflow repair + final critical-task qualification (Phase 7D.3D)
uv run research-agent routing qualify --role reasoning --repetitions 3 --tasks gap_analysis --candidates "<slug>"
uv run research-agent routing qualification remaining                    # gap_analysis now covered (gemini-3.7-flash primary)
uv run research-agent eval run research-gap-analysis-v1                  # offline gap metrics unchanged after the live repair

# Candidate refresh + final qualification sweep (Phase 7D.3E)
uv run research-agent routing preflight --config <approved-config>      # capability preflight of all approved models
uv run research-agent routing qualify --role critic --repetitions 3 --candidates "<approved-slugs>"
uv run research-agent routing qualify --role fast --repetitions 3 --candidates "<approved-slugs>"
uv run research-agent routing qualification remaining                   # final coverage matrix (persisted)

# Task-aware shadow routing (Phase 7D.4)
uv run research-agent routing shadow-task --role reasoning --task gap_analysis   # one advisory (role, task) decision
uv run research-agent routing shadow-campaign                                    # all roles/tasks over the current matrix
uv run research-agent eval run task-aware-shadow-routing-v1                      # offline routing benchmark (unsafe_task_route_rate=0)
```

Model-routing infrastructure and task-aware shadow routing are implemented, but model qualification and production activation are intentionally incomplete. See [docs/model-routing-status.md](docs/model-routing-status.md).
...
- **Phase 7D.4** — task-aware shadow routing: the shadow router now selects an advisory model per (role, task) from the exact-task TaskQualificationMatrix (never transferred across tasks), instead of a single role-level leaderboard. `routing.shadow-task` / `routing.shadow-campaign` run the router over the current qualification matrix and persist immutable `task_aware_routing_decision` artifacts (task/role, current static model, qualified candidates, selected candidate, fallback with `fallback_not_live_qualified` when the fallback is the static model, qualification evidence ids, decision policy, would_switch + expected quality/latency/cost deltas). Uncovered tasks stay on the configured static role model (`static_fallback`, reason `no_qualified_task_model`; stale evidence `stale_qualification`; role/task mismatch rejected) — never an error, never a dynamic switch. Production routing remains DISABLED. Offline benchmark `task-aware-shadow-routing-v1` (11 cases) + `evaluator.task_aware_routing` with the critical invariant **unsafe_task_route_rate = 0**. Shadow campaign over the live matrix: evidence_extraction and gap_analysis would switch to gemini-3.7-flash (static nemotron, fallback not live qualified); synthesis, mechanism/model/proposition generation and screening stay static (no qualified model)
- **Phase 7D.3E** — candidate refresh + final qualification sweep: the active qualification config was reset to the operator-approved 10-model pool (stale candidates deactivated; slugs stay config-driven, never in service logic). All 10 preflighted and persisted (requested/resolved/availability/structured-output/context/latency); only preflight-passing models entered campaigns. **Genuine defect found + fixed**: `AnalyticalModelOpportunity.domains` used an under-specified eight-domain enum whose validation CRASHED the mechanism pipeline whenever a model named a legitimate field such as "industrial organization" or "entry deterrence" (observed as hard case errors for deepseek/gemini-3.7-flash). The vocabulary was expanded to a comprehensive legitimate analytical IS/economic domain set (bogus domains still rejected) and the mechanism-critic revision path now degrades gracefully to the documented "original candidate selected unchanged" fallback instead of crashing. Regression tests cover the vocabulary, the fallback, the approved-config invariant (no stale slugs), provider-unavailable exclusion, primary/fallback-from-qualified-only selection, and the no-unsafe-qualification threshold invariant. Sweep (approved models, >=3 reps, 5 for borderline): **gap_analysis and evidence_extraction remain qualified for gemini-3.7-flash (det 1.0); mechanism/model-spec/proposition critique qualified for gemini-3.7-flash (mechanism_critique also qwen3.8-flash det 1.0)**. No qualified primary emerged for synthesis, mechanism_generation, model_specification, proposition_generation, results_critique, manuscript_critique, or screening (dominant below_quality_threshold; structural validity/defect-recall/decision-accuracy, not provider availability). The sweep demonstrates genuine task specialization (gemini-3.7-flash for gap/evidence/critique; qwen3.8-flash for critique quality but provider-variance; no approved model close on generation/synthesis/screening)
- **Phase 7D.3D** — gap-workflow repair + final critical-task qualification: the live gap workflow reused stable fixture ids with idempotent `_put_explicit`, so every repetition after the first had fixtures not "produced by this run" and correct gaps were flagged as unsupported. Repaired with a run-scoped ID mapping layer: live (real-model) runs use run-unique fixture ids with fresh puts (and now produce the `paper_identity` artifacts the profiles reference); offline research-gap-analysis-v1 keeps stable ids and idempotency — its metrics are byte-identical (9/10 with one designed-failure; gap_precision 0.889, gap_recall 1.0, grounding_accuracy 0.889). GapAnalyzerService and evaluator grounding rules untouched. Regression tests cover distinct per-rep fixture ids, remapped refs, no stale reuse, correct-output grounding pass, hallucinated-id fail, offline stability, and SQLite-reopen provenance. **Authoritative re-qualification (allowed model set only): gap_analysis now QUALIFIED for google/gemini-3.7-flash (det 1.0, 3/3)**. Model set restricted to the approved list (z-ai/glm-5.2:free, nemotron-3-ultra:free, minimax-m3:free, gemma-4-31b-it:free, qwen3.8-flash, glm-5.3-flash, gemini-3.7-flash, deepseek-v4-flash, qwen3.7-flash, gpt-5.6-luna); preflight: 8 available (gemini-3.7-flash, deepseek, minimax, nemotron, qwen3.7/3.8-flash, glm-5.3-flash, gpt-5.6-luna), glm-5.2:free temporarily_unavailable, gemma-4 capability_mismatch. gemini-3.7-flash also qualifies evidence_extraction and mechanism/model-specification/proposition critique. Still uncovered: mechanism_generation, model_specification, proposition_generation, results_critique, manuscript_critique, screening (dominant below_quality_threshold; qwen3.8-flash critic det 0.933 but provider-gated 0.32 > 0.1)
- [ ] Resume Phase 7 model qualification and controlled production routing activation.
